...
class Store extends command_1.SfdxCommand {
    ...
    async saveAuthInfo(authInfo) {
        await authInfo.save();
        await common_1.Common.handleSideEffects(authInfo, this.flags);
        await common_1.Common.identifyPossibleScratchOrgs(authInfo.getFields(true), authInfo);
    }
    async overwriteAuthInfo(username) {
        if (!this.flags.noprompt) {
            const authInfoConfig = await core_1.AuthInfoConfig.create({
                ...core_1.AuthInfoConfig.getOptions(username),
    ...
    async getAccessToken() {
        ...
        if (!core_1.sfdc.matchesAccessToken(accessToken)) {
            throw new core_1.SfdxError(messages.getMessage('invalidAccessTokenFormat', [ACCESS_TOKEN_FORMAT]));
        }
        return accessToken;
    }
}
