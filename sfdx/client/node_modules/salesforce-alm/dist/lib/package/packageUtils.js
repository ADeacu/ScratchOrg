...
const ANCESTRY_ENHANCEMENTS_MIN_VERSION = '54.0';
...
module.exports = {
    ...
    async validatePatchVersion(force, org, versionNumberString, packageId) {
        ...
        if (queryResult.records === null || queryResult.records.length === 0) {
            throw core_1.SfdxError.create('salesforce-alm', 'packaging', 'errorInvalidPackageId', [packageId]);
        }
    ...
    async getPackage2Type(package2Id, force, org) {
        ...
        if (!queryResult || queryResult.records === null || queryResult.records.length === 0) {
            throw core_1.SfdxError.create('salesforce-alm', 'packaging', 'errorInvalidPackageId', [package2Id]);
        }
    ...
    async getPackage2TypeBy04t(package2VersionId, force, org, installKey) {
        ...
        const queryResult = await force.toolingQuery(org, query);
        if (!queryResult || queryResult.records === null || queryResult.records.length === 0) {
            throw core_1.SfdxError.create('salesforce-alm', 'packaging', 'errorInvalidPackageId', [package2VersionId]);
        }
    ...
    /**
     * Temp method needed until 236 is on instances
     */
    async retrieveMaxApiVersion(org) {
        const orgMethods = await core_1.Org.create({
            connection: await core_1.Connection.create({
                authInfo: await core_1.AuthInfo.create({ username: org.getName() }),
            }),
        });
        return orgMethods.retrieveMaxApiVersion();
    },
    /**
    ...
    getAncestorId(packageDescriptorJson, force, org, versionNumberString, skipAncestorCheck) {
        return Promise.resolve().then(async () => {
            ...
            // determine if ancestryCheck should be enabled
            const maxApiVersionForOrg = await this.retrieveMaxApiVersion(org);
            const ancestryCheckEnabled = maxApiVersionForOrg >= ANCESTRY_ENHANCEMENTS_MIN_VERSION;
            const versionNumber = versionNumberString.split(VERSION_NUMBER_SEP);
            ...
            if (!ancestryCheckEnabled) {
                explicitUseHighestRelease = false;
                explicitUseNoAncestor = false;
            }
            else if (explicitUseNoAncestor && skipAncestorCheck) {
                return '';
            }
            else {
                let result = await this.getAncestorIdHighestRelease(force, org, packageId, versionNumber, versionNumberString, isPatch, explicitUseHighestRelease, skipAncestorCheck);
            ...
            if (!ancestryCheckEnabled && !packageDescriptorJson.ancestorVersion) {
                return ancestorId;
            }
    ...
    async getAncestorIdHighestRelease(force, org, packageId, versionNumber, versionNumberString, isPatch, explicitUseHighestRelease, skipAncestorCheck) {
        let result = { finalAncestorId: null, highestReleasedVersion: null };
        if (isPatch && explicitUseHighestRelease) {
            ...
            if (highestVersionRecords && (highestVersionRecords === null || highestVersionRecords === void 0 ? void 0 : highestVersionRecords.length) == 1 && highestVersionRecords[0]) {
                ...
                throw new Error(messages.getMessage('errorNoMatchingAncestor', versionNumberString, 'packaging'));
            }
        }
        return result;
    },
    ...
    ANCESTRY_ENHANCEMENTS_MIN_VERSION,
};
